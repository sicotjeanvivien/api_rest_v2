use crate::{
    application::{CredentialHasher, UserService},
    domain::{NewUser, RepositoryError, User, UserAuth, UserRegister},
};
use std::sync::Arc;
...
impl CredentialService {
    ...
    pub(crate) async fn login(&self, user_auth: UserAuth) -> Result<User, RepositoryError> {
        let user = self
            .user_service
